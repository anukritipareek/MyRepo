...
echo ""
echo "🚀 APEX TEST CLASS IDENTIFICATION"
echo "=========================================="
...
if [ "${HAS_DEPLOYMENT_PACKAGE:-true}" = "false" ]; then
  ...
  echo '{"result":"No deployment package","classes":[],"tests":[]}' 
...
if [ ! -f "changed-sources/package/package.xml" ]; then
  ...
  echo '{"result":"No package.xml found","classes":[],"tests":[]}' 
  echo "RELATED_TESTS=" >> "$GITHUB_ENV"
  exit 0
fi
...
package_path = Path("changed-sources/package/package.xml")
apex_classes = []
...
if [ -z "$DELTA_APEX_CLASSES" ]; then
  ...
  echo '{"result":"No Apex classes in delta","classes":[],"tests":[]}' 
  echo "RELATED_TESTS=" >> "$GITHUB_ENV"
  exit 0
fi
...
if [ ! -d "changed-sources/force-app/main/default/classes" ]; then
  ...
  echo "{\"result\":\"Metadata-only deployment\",\"classes\":[],\"tests\":[]}"
  echo "RELATED_TESTS=" >> "$GITHUB_ENV"
  exit 0
fi
...
for cls in $DELTA_APEX_CLASSES; do
  file_path=$(find changed-sources/force-app force-app -path "*/classes/${cls}.cls" 2>/dev/null | head -n1)
...
if [ -n "$ALL_CLASSES" ] && [ -n "$ALL_TESTS" ]; then
  ...
  for cls in $ALL_CLASSES; do
    for test in $ALL_TESTS; do
      ...
      test_file=$(find changed-sources/force-app force-app -path "*/classes/${test}.cls" 2>/dev/null | head -n1)
